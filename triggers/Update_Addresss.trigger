Trigger Update_Addresss on Company__c (after update,before delete) {
    ...
    if(Trigger.isafter && Trigger.isupdate){
        ...
        list<employee__c> epList=[select id, Address__c, company1__c, company2__c, company1__r.Address__c, company2__r.Address__c from employee__c where company1__r.id IN:idList OR company2__r.id IN:idList];
        if (epList.size() > 0 )
        {
            for ( Employee__c employee : epList )
            {
                employee.Address__c = employee.company1__r.Address__c + ',' + employee.company2__r.Address__c;
    ...
    if(Trigger.isbefore && Trigger.isdelete){
        ...
        if (epList.size() > 0 )
        {
            for ( Employee__c employee : epList )
            {
                ...
                else if (employee.company2__r.Address__c== null)
                {
